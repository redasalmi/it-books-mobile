...
const booksSlice = createSlice({
  ...
  reducers: {
    fetchBooksSucceeded(state, action) {
      const {books, bookSearch} = action.payload;
      const totalBooks = parseInt(books.total);
      return {
        ...state,
...
const fetchBooks = (search = '', page = 1) => async (dispatch) => {
  try {
    ...
    const payload = {books: books, bookSearch: search};
    dispatch(fetchBooksSucceeded(payload));
  } catch (error) {
